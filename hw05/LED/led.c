 ...
 * The sysfs entry appears at /sys/ebb/gpio60
...
static unsigned int gpioLED1 = 60, gpioLED2 = 48;           ///< Default GPIO for the LED is 60
module_param(gpioLED1, uint, S_IRUGO);       ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(gpioLED1, " GPIO LED number (default=60)");     ///< parameter description

static unsigned int blinkPeriod1 = 1000, blinkPeriod2 = 2000;     ///< The blink period in ms
...
static bool ledOn1 = 0, ledOn2 = 0;                      ///< Is the LED on or off? Used for flashing
 ...
 */
static struct kobj_attribute period_attr = __ATTR(blinkPeriod1, 0664, period_show, period_store);
 ...
 *  case it is gpio60, which is automatically defined in the ebb_LED_init() function below
 *  using the custom kernel parameter that can be passed when the module is loaded.
 */
static struct attribute_group attr_group = {
   .name  = ledName,                        // The name is generated in ebb_LED_init()
   .attrs = ebb_attrs,                      // The attributes array defined just above
};

static struct kobject *ebb_kobj;            // The pointer to the kobject
static struct task_struct *task1, *task2;            // The pointer to the thread task
...
static int __init ebb_LED_init(void){
   ...
   sprintf(ledName, "led%d", gpioLED1);      // Create the gpio name for /sys/ebb/led60
   ...
   // add the attributes to /sys/ebb/ -- for example, /sys/ebb/led60/ledOn1
   ...
   ledOn1 = true;
   ledOn2 = true;
   gpio_request(gpioLED1, "sysfs");          // gpioLED1 is 60 by default, request it
   ...
   gpio_request(gpioLED2, "sysfs");          // gpioLED1 is 60 by default, request it
   gpio_direction_output(gpioLED2, ledOn2);   // Set the gpio to be in output mode and turn on
   gpio_export(gpioLED2, false);  // causes gpio60 to appear in /sys/class/gpio
                                 // the second argument prevents the direction from being changed

   task1 = kthread_run(flash1, NULL, "LED2_flash_thread");  // Start the LED flashing thread
   task2 = kthread_run(flash2, NULL, "LED1_flash_thread");  // Start the LED flashing thread
...
static void __exit ebb_LED_exit(void){
   ...
   printk(KERN_INFO "EBB LED: Goodbye from the Beagle LED LKM!\n");
}

/// This next calls are  mandatory -- they identify the initialization function
/// and the cleanup function (as above).
module_init(ebb_LED_init);
module_exit(ebb_LED_exit);
